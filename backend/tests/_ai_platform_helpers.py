...
def client_for(Session, uid):
    from backend.app.routes.ai_platform import ROUTERS
    app = FastAPI()
    for r in ROUTERS:
...
def seed_assessment(db, *, user_id=1, **overrides):
    base = dict(user_id=user_id, industry="general", business_type="pvt",
                years_in_business=10, employee_count=150, country="IN",
